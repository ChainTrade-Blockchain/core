...
func setup(withGenesis bool, invCheckPeriod uint, chainID string, evmChainID uint64) (*CVMD, GenesisState) {
	db := dbm.NewMemDB()
...
// Setup initializes a new CVMD. A Nop logger is set in CVMD.
func Setup(t *testing.T, chainID string, evmChainID uint64) *CVMD {
	t.Helper()
...
// SetupWithGenesisValSet initializes a new CVMD with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the simapp from first genesis
// account. A Nop logger is set in CVMD.
func SetupWithGenesisValSet(t *testing.T, chainID string, evmChainID uint64, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *CVMD {
	t.Helper()
