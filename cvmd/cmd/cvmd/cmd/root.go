...
func NewRootCmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		Use:   "cvmd",
		Short: "exemplary ChainTrade EVM app",
...
func initRootCmd(rootCmd *cobra.Command, osApp *cvmd.CVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	if height != -1 {
		exampleApp = cvmd.NewExampleApp(logger, db, traceStore, false, appOpts, evmdconfig.EVMChainID, evmdconfig.EvmAppOptions, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = cvmd.NewExampleApp(logger, db, traceStore, true, appOpts, evmdconfig.EVMChainID, evmdconfig.EvmAppOptions, baseapp.SetChainID(chainID))
	}
